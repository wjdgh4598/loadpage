...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    dropHandler: function (event) {
      ...
      if (!$dropZone.siblings('.nodes').length) { // if the drop zone is a leaf node
        ...
        if ($dropZone.children('.title').length) {
          $dropZone.children('.title').prepend('<i class="oci"></i>');
        }
      } else {
    ...
    createNode: function (data) {
      ...
      //조직도에 사진가져오기
      var noImg = "/assets/images/jpg/dummy-photo@2x.jpg";
      var exImg = "/files/"+ data.photoFileId+"/"+data.fileSn+"/download";
      var orgEmplPhoto = '<img id="_orgChartPhoto_img" src=\''+exImg+'\' style="width:50px; height:60px; object-fit: cover;" onerror="this.src=\''+noImg+'\'" />';
      var positionGradeNameHan =  data[opts.nodeTitle] === "" ? "" : data.positionNameHan + '/' + data.gradeNameHan;

          $nodeDiv.append('<div class="title">' + data[opts.nodeContent] + '</div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">'
              + orgEmplPhoto
              + (data[opts.nodeTitle] || '') + '<br>'
              + '<span>' + positionGradeNameHan + '<br>'
              + '</span></div>' : '');
      }
      //
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        ...
        if(Number(flags.substr(2,1))) {
          $nodeDiv.append('<i class="edge verticalEdge bottomEdge oci"></i>')
            .children('.title').prepend('<i class="oci"></i>');
        }
      }
    ...
    addChildren: function ($node, data) {
      ...
      if (!$node.find('.symbol').length) {
        $node.children('.title').prepend('<i class="oci"></i>');
      }
      if (this.isInAction($node)) {
        this.switchVerticalArrow($node.children('.bottomEdge'));
      }
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      //console.log("that.options.direction===>"+that.options.direction);
      //console.log("flag===>"+flag);
      //console.log("sourceChart.clientWidth==>" + sourceChart.clientWidth);
      //console.log("sourceChart.clientHeight==>" + sourceChart.clientHeight);
