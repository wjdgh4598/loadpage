...
(c) Dinamenta, UAB.
*/
scheduler.templates.calendar_month = scheduler.date.date_to_str("%Y년 %F "),
...
scheduler.renderCalendar = function (e, t, a) {
    ...
    if (scheduler.config.minicalendar.mark_events) for (
        var s = scheduler.date.month_start(n), o = scheduler.date.add(s, 1, "month"), _ = this.getEvents(s, o), c = this["filter_" + this._mode], u = 0; u < _.length; u++) {
...
}, scheduler.attachEvent("onTemplatesReady", function () { scheduler._attach_minical_events() }), scheduler.templates.calendar_time = scheduler.date.date_to_str("%Y-%m-%d"), scheduler.form_blocks.calendar_time = { render: function () {
...
e.value = scheduler.templates.calendar_time(t), e._date = scheduler.date.date_part(new Date(t))
        ...
        e.onclick = function () {
            scheduler.destroyCalendar(null, !0), scheduler.renderCalendar({ position: e, date: new Date(this._date), navigation: !0, handler: function (t) {
